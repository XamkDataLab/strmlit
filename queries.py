...
def fetch_data_new(search_term, search_by='y_tunnus'):
    if search_by == 'y_tunnus':
        where_clause = "y.y_tunnus = ?"
    elif search_by == 'company_name':
        where_clause = "y.yritys LIKE ?"
        search_term = f"%{search_term}%"  # Add wildcard for partial matching
    else:
        raise ValueError("search_by must be 'y_tunnus' or 'company_name'")

    # The SQL query with the WHERE clause based on the search_by option
    query = f"""
    WITH Funding AS (
        SELECT 
            Y_tunnus,
